...
from dataset import UltraDataset_ds
import model
import utils

if __name__ == "__main__":
    """ 准备数据 """
    train_dataset = UltraDataset_ds("data/train.txt", image_height=512, image_weight=512, image_aug=False)
    val_dataset = UltraDataset_ds("data/val.txt", image_height=512, image_weight=512, image_aug=False)
    ...
    net = model.Attention_ds_Unet_Vgg.Attention_Unet_ds_Vgg(num_class=2, in_channels=3)
    net = net.to(device)
    ...
    for epoch in range(epochs):
        ...
        for iteration, (imgs, label_512, label_256, label_128) in pbar:
            # 简单处理
            with torch.no_grad():
                imgs = imgs.to(device).float() / 255.0
                label_512 = label_512.to(device).float().long()
                label_256 = label_256.to(device).float().long()
                label_128 = label_128.to(device).float().long()

            # 模型推理
            optimizer.zero_grad()
            pred_512, pred_256, pred_128 = net(imgs)

            loss = loss_func(pred_512, label_512) + loss_func(pred_256, label_256) + loss_func(pred_128, label_128)
            # 反向传播
            loss.backward()
            optimizer.step()
            ...
            with torch.no_grad():
                # 只用512 or 还是取平均
                accs, miou, mdice, kappas = utils.calculate_metrics(pred_512.cpu(), label_512.cpu())
        ...
        for iteration, (imgs, label512, _, _) in pbar:
            with torch.no_grad():
                # 简单处理
                imgs = imgs.to(device).float() / 255.0
                label512 = label512.to(device).float().long()
                # 模型推理
                optimizer.zero_grad()
                preds_512, _, _ = net(imgs)
                # caculate the metrics,  acc iou dice
                accs, miou, mdice, kappas = utils.metrics.calculate_metrics(preds_512.cpu(), label512.cpu())
                # caculate the epoch evaluation avg loss
                val_loss = loss_func(preds_512, label512)
