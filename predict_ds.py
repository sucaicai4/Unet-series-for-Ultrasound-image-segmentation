...
from dataset import UltraDataset, UltraDataset_ds
import model
import utils
...
if __name__ == "__main__":
    """ 准备数据 """
    test_dataset = UltraDataset_ds("data/test.txt", image_height=512, image_weight=512, image_aug=False)
    ...
    weight = "epoch20_total_loss0.6528_val_loss0.4179.pth"
    ...
    for iteration, (imgs, label512, _, _) in pbar:
        with torch.no_grad():
            ...
            label512 = label512.long()
            # 模型推理
            preds, _, _ = net(imgs)
            accs, miou, mdice, _ = utils.metrics.calculate_metrics(preds.cpu(), label512)
            preds = torch.argmax(preds, dim=1).float()

            preds_np = preds.cpu().numpy()[0]
            label_np = label512.numpy()[0]
